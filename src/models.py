...
from preprocessing import AntFreq2BeamTime_3D, BeamDelay2AntFreq_3D, Pol2Dim, PowerNoiseNormalizer, CenterCut2
from plotting import project3d
...
class DnCNN3D(nn.Module):
    def __init__(self, channels, num_of_layers = 17, apply_kaiming = True):
...
class Beam_DnCNN_3D(torch.nn.Module):
    def __init__(self, cfg, layers = 20):
        super(Beam_DnCNN_3D, self).__init__()

        self.Denoiser = DnCNN3D(4, layers)
        self.BeamTransform = AntFreq2BeamTime_3D(n_hor = cfg['N_hor'],
                                                 n_ver = cfg['N_ver'],
                                                 n_pol = cfg['N_pol'],
                                                 upsampling = (cfg['N_Az'], cfg['N_El'], cfg['N_time']),
                                                 shifts = cfg['shifts'])
    ...
    def dim_to_complex(self,x):
        return (x[:,0:2,:,:] + 1j*x[:,2:4,:,:])# =============================torch.unsqueeze((x[:,0,:,:,:] + 1j*x[:,1,:,:,:]), dim = 1) 
